...
<xsl:stylesheet version="1.0" xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance" xmlns:mods="http://www.loc.gov/mods/v3" exclude-result-prefixes="mods" xmlns:dc="http://purl.org/dc/elements/1.1/" xmlns:srw_dc="info:srw/schema/1/dc-schema" xmlns:oai_dc="http://www.openarchives.org/OAI/2.0/oai_dc/" xmlns:xsl="http://www.w3.org/1999/XSL/Transform">
	...
	Version 2.0     2023-08-11 wteal@iastate.edu
					...
					Added '<xsl:value-of select="."/>' to make the behavior for dc:contributor more consistent
						with that for dc:creator.
					For dc:subject replaced double-dash with '; ' to appropriately concatenate subjects.
					Added '<xsl:if test="position()!=last()">; </xsl:if>' to make the handling of names
						consistent with the handling of other subject terms.
					Added '<xsl:if test="position()!=last()">; </xsl:if>' to dc:subject to make the handling
						of names consistent with the handling of other subject terms so they're not mashed
						together like 'OrganizationOther Organization'
					Added '<xsl:if test="position()!=last()">; </xsl:if>' to dc:subject to make the handling
						of geographic terms consistent with the handling of other subject terms.
					For dc:date replaced ' - ' and '-' as range indicators with '/' to conform to ISO 8601.
					Fixed variable names causing Oxygen to throw "Bad practice naming variables the same as
						elements and attributes from XPath expressions" warnings.
	...
	This stylesheet transforms MODS version 3.x records and collections of records to simple Dublin Core (DC) records, 
	based on the Library of Congress' MODS to simple DC mapping <http://www.loc.gov/standards/mods/mods-dcsimple.html> 
			
	The stylesheet will transform a collection of MODS 3.x records into simple Dublin Core (DC)
	as expressed by the SRU DC schema <http://www.loc.gov/standards/sru/dc-schema.xsd>
	
	The stylesheet will transform a single MODS 3.x record into simple Dublin Core (DC)
...
-->
	...
	<xsl:template match="mods:name">
		<!--Added if test to not output empty elements.-->
		<xsl:if test="normalize-space(mods:namePart/text())!= ''">
			<xsl:choose>
				<xsl:when test="mods:role/mods:roleTerm[@type='text']='creator' or mods:role/mods:roleTerm[@type='code']='cre' ">
					<dc:creator>
						...
						<xsl:choose>
							...
							<!--Removed 'et al', so as not to tack it on unnecessarily when it's not relevant.-->
							<xsl:otherwise>
								<xsl:text/>
							</xsl:otherwise>
						</xsl:choose>
				...
				<xsl:otherwise>
					<!-- ws  1.7 -->
					<dc:contributor>
						<!--Removed 'et al.' (note the added period that makes it inconsistent with the 'et al' that was used for dc:creator.
							Added '<xsl:value-of select="."/>' to make the behavior for dc:contributor more consistent with that for dc:creator.-->
						<xsl:call-template name="name"/>
						<xsl:if test="mods:etal"><xsl:value-of select="."/></xsl:if>
					</dc:contributor>
				</xsl:otherwise>
			</xsl:choose>
		</xsl:if>
			...
			<xsl:if test="normalize-space(.)!= ''">
				<dc:subject>
					<xsl:for-each select="mods:topic | mods:occupation">
						<xsl:value-of select="."/>
						<!--Replaced double-dash with '; ' to appropriately concatenate subjects-->
						<xsl:if test="position()!=last()">; </xsl:if>
					</xsl:for-each>
					<xsl:for-each select="mods:name">
						<xsl:call-template name="name"/>
						<!--Added '<xsl:if test="position()!=last()">; </xsl:if>' to make the handling of names
							consistent with the handling of other subject terms so they're not mashed together like
							'OrganizationOther Organization'-->
						<xsl:if test="position()!=last()">; </xsl:if>
					</xsl:for-each>
				</dc:subject>
			</xsl:if>
			...
			<dc:date>
				<xsl:choose>
					<xsl:when test="@point='start'">
						<xsl:value-of select="."/>
						<!--Replaced ' - ' with '/' to conform to ISO 8601.-->
						<xsl:text>/</xsl:text>
					</xsl:when>
	...
	<xsl:template match="mods:dateIssued[@point='start'] | mods:dateCreated[@point='start'] | mods:dateCaptured[@point='start'] | mods:dateOther[@point='start'] ">
		...
		<xsl:if test="normalize-space(.)!= ''">
			<dc:date>
				<!--Replaced '-' (note dash without spaces was inconsitent with how it was handled in the above date template) .-->
	...
	<xsl:template match="mods:physicalDescription">
		<xsl:for-each select="mods:extent | mods:form | mods:internetMediaType">
			<dc:format>
				<!-- tmee mods 3.5 -->
				<xsl:variable name="mods_unit" select="translate(@unit,'ABCDEFGHIJKLMNOPQRSTUVWXYZ','abcdefghijklmnopqrstuvwxyz')"/>
				<!-- ws 1.7 -->
				<xsl:if test="@unit">
					<xsl:value-of select="$mods_unit"/>: 
				</xsl:if>
				<xsl:value-of select="."/>
			</dc:format>
		</xsl:for-each>
	...
	<xsl:template match="mods:identifier">
		...
		<xsl:if test="normalize-space(.)!= ''">
			<dc:identifier>
				<xsl:variable name="mods_type" select="translate(@type,'ABCDEFGHIJKLMNOPQRSTUVWXYZ','abcdefghijklmnopqrstuvwxyz')"/>
				<xsl:choose>
					...
					<xsl:when test="@type">
						<xsl:choose>
							<xsl:when test="@type">
								<xsl:value-of select="$mods_type"/>: <xsl:value-of select="."/>
							</xsl:when>
							<xsl:when test="contains ('isbn issn uri doi lccn uri', $mods_type)">
								<xsl:value-of select="$mods_type"/>: <xsl:value-of select="."/>
							</xsl:when>
						</xsl:choose>
					</xsl:when>
